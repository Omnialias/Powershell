...
#
# Prompt for user info and account creation
#
Do {
    Try {
        ...
        $CreateUserError = Show-AnyBox -Icon 'Question' -Title 'User Exists' -Message "Username already exists. Please try again" -Buttons 'Continue','Cancel' -MinWidth 300
        If ( $CreateUserError.Cancel -eq $True) { Break }
        $prompts = $null
    }
} while (
    $prompts -eq $null
...
If ($Prompts.Department -eq 'CBS') {
    Invoke-Command -ComputerName NAS4 -ScriptBlock {
        New-Item -Path $Using:CreateFolder -ItemType Directory
...
Invoke-Command -ComputerName TDCDocs -ScriptBlock {
    New-Item -Path $Using:ScanFolder -ItemType Directory
...
if ( $HostpilotPrompt.No -eq $True ) { Break }
...
# Send Welcome E-mail
# 
$EmailBody = Get-Content C:\Scripts\Active\NewUserScript\WelcomeEmail.txt | Out-String
...
if ( $SecuriSyncPrompt.Yes -eq $True ) { Enable-FileUser -Identity $CPUser }
...
Show-AnyBox -Title 'Double-Check' -Buttons 'Looks Good' -Message 'Double check items below',' ', `
    "AD Items", `
    $ADCheck.Name, `
